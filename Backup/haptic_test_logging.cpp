...
public:		Output<typename boost::tuple<cf_type> > output;
protected:	Output<typename boost::tuple<cf_type> >::Value* outputValue;
...
protected:
	virtual void operate() {
        ...
        outputValue->setData(&ftsIn);  // Push data into the output
	}
...
int wam_main(int argc, char** argv, ProductManager& pm, systems::Wam<DOF>& wam) {
    ...
    systems::TupleGrouper<jp_type, typename boost::tuple<cf_type> > tg;
    typedef boost::tuple<jp_type, typename boost::tuple<cf_type> > tuple_type;
    ...
    connect(wam.jpOutput, tg.template getInput<0>());
